...
public class MondoCommand implements CommandExecutor, SubHandler {
	 ...
	 * 
	 * @param sender       A CommandSender who is the person or console who sent
	 *                     this command.
	...
	private void handleRawCommand(CommandSender sender, Player player, String commandLabel, List<String> args) {
		...
		if (sub == null) {
			showUsage(sender, player, commandLabel);
			return;
		} else if (!sub.checkPermission(sender) || !sub.checkRank(sender)) {
			/*
			 * Core.getInstance().log("handleRawCommand Name: " + sub.getName() +
			 * " Permission: " + sub.getPermission() + " Rank: " + sub.getRank(), true);
			 */
	 ...
	 * @param slash        An empty string if there should be a slash prefix, a
	 *                     slash otherwise.
	 */
	 ...
	 * @param permission The permission string of a permission to check for this
	 *                   command.
	 * @return a new SubCommand.
	 */
	@Deprecated
	public SubCommand addSub(String name, String permission, Rank rank) {
		if (permission != "") {
			SubCommand cmd = new SubCommand(name, permission, rank).setHandler(fallbackHandler);
			subcommands.put(name.toLowerCase(), cmd);
			return cmd;
		} else {
			SubCommand cmd = new SubCommand(name, rank).setHandler(fallbackHandler);
			subcommands.put(name.toLowerCase(), cmd);
			return cmd;
		}
	}
	 ...
	 * 
	 * @param name The name of this sub-command.
	 * @param rank The rank to check for this command.
	 * @return a new SubCommand.
	 */
	public SubCommand addSub(String name, Rank rank) {
		SubCommand cmd = new SubCommand(name, rank).setHandler(fallbackHandler);
		subcommands.put(name.toLowerCase(), cmd);
		return cmd;
	}
	...
	private List<SubCommand> availableCommands(CommandSender sender, Player player) {
		...
		for (SubCommand sub : subcommands.values()) {
			/*
			 * Core.getInstance().log("Command: " + sub.getName() + " Permission: " +
			 * sub.getPermission() + " RankRequired: " + sub.getRank(), true);
			 */
			if ((has_player || sub.isConsoleAllowed()) && (sub.checkPermission(sender) || sub.checkRank(sender))) {
				items.add(sub);
			}
		}
		return items;
